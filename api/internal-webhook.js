...
// Tool handlers (full access, no PIN required)
const handlers = {
...
// Extract tool name/args helpers
  ...
  try {
    ...
    if (message?.type === 'tool-calls') {
      ...
      for (const tc of toolCalls) {
        ...
        const result = fn ? await fn(args) : { error: `Unknown tool: ${name}`, summary: `I don't have a tool called ${name}` };
